"""Persistent parallel FLORIS evaluator for GA populations and wind scenarios.

Place this file in your ``wrapper`` package as ``parallel_floris.py``.
Worker functions remain at module scope for Windows multiprocessing.
"""
...
def _initialize_floris_worker(config_path: str, turbine_type: str) -> None:
    """Create one reusable FLORIS model inside each worker process."""
...
def _evaluate_layout_worker(task):
    """Evaluate all paired wind scenarios for one candidate layout."""
    global _WORKER_FMODEL, _WORKER_TURBINE_TYPE
...
class ParallelFlorisEvaluator:
    """Persistent process-pool wrapper for repeated FLORIS population evaluation.

    Create one instance before the GA loop, reuse it for every generation, then
    call ``close()`` when optimization is finished. It can also be used as a
    context manager.
    """
    ...
    def start(self) -> None:
        """Start the persistent process pool if it is not already running."""
        if self._executor is None:
            self._executor = ProcessPoolExecutor(
                max_workers=self.max_workers,
                initializer=_initialize_floris_worker,
                initargs=(str(self.config_path), self.turbine_type),
            )
    ...
    def __enter__(self) -> "ParallelFlorisEvaluator":
        self.start()
        return self
    ...
    def evaluate_population(
        ...
        *,
        ...
        progress_desc: str = "FLORIS layouts",
        ...
        if self._executor is None:
            raise RuntimeError(
                "The process pool is closed. Call start() before evaluation."
            )
        ...
        if turbulence_intensities.size != wind_speeds.size:
            raise ValueError(
                "turbulence_intensities must be scalar or contain one value "
                "per scenario."
            )
        ...
        waked_velocities = np.empty(
            (pop_size, n_scenarios, n_turbines), dtype=np.float64
        )
        ...
        effective_chunksize = self.chunksize if chunksize is None else chunksize
        effective_progress = (
